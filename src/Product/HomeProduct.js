...
export default function HomeProduct() {
  ...
  async function getProduct() {
    const response = await axios.get(
      `http://localhost:8080/api/shops/${idShop}`
    );
  ...
  async function CreateOrder(e) {
    ...
    try {
      const orderResponse = await axios.post(
        `http://localhost:8080/api/order/${idShop}/${idUser}`,
        null,
        { timeout: 5000 }
      );
  ...
  async function getMenu() {
    const response = await axios.get(
      `http://localhost:8080/api/menus/${idShop}`
    );
    console.log(response.data);
  ...
  const fetchData = async () => {
    try {
      ...
      const hasProducts = menuProductsData.some(
        (menuProduct) => menuProduct.products.length > 0
      );
  ...
  const findByNameAndMenu = async (menuId, productName) => {
    try {
      const productResponse = await axios.get(
        `http://localhost:8080/api/products/FindByPByName/${menuId}?productName=${productName}`
      );
  ...
  const Showcar = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/detailCart/${idShop}/${idUser}`
      );
  ...
  const addProductToCart = async (idShop, idUser, idProduct) => {
    try {
      const response = await axios.post(
        `http://localhost:8080/api/detailCart/${idUser}/${idShop}/${idProduct}`
      );
  ...
  const checkUserLikeExists = async (userId, productId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/products/like-exists?userId=${userId}&productId=${productId}`
      );
      if (!response.ok) {
  ...
  const handleLike = async (userId, productId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/products/plus-like?userId=${userId}&productId=${productId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleUnlike = async (userId, productId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/products/delete-like?userId=${userId}&productId=${productId}`,
        {
          method: "DELETE",
        }
      );
          ...
          <div className="row">
            ...
            <div className="col">
              <div className="detail-restaurant-info">
                ...
                <div className="status-restaurant">
                  <span>
                    <FontAwesomeIcon icon={faLocationDot} className="faClock" />
                    {address}
                  </span>
                </div>
                <div className="status-restaurant">
                  <span>
                    <FontAwesomeIcon icon={faPhone} className="faClock" />
                    {phoneNumber}
                  </span>
                </div>
                <div className="status-restaurant">
                  <span>
                    <FontAwesomeIcon icon={faEnvelope} className="faClock" />
                    {email}
                  </span>
                </div>
                <div className="cost-restaurant">
                  <span>
                    <FontAwesomeIcon icon={faSackDollar} /> 10.000 - 100.000
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="menu-restaurant-category">
                <div className="list-category">
                  <div className="scrollbar-container ps">
                    <div className="item">
                      {menus.map((menu) => (
                        <span className="item-link" key={menu.id}>
                          {menu.name}
                        </span>
                      ))}
                    </div>
                                  ...
                                  >
                                    <div className="row">
                                      ...
                                      <div className="col item-restaurant-info">
                                        ...
                                        <div className="item-restaurant-desc">
                                          {product.detail}
                                        </div>
                                        <div>
                                          {likedProducts[product.id] ? (
                                            <FontAwesomeIcon
                                              ...
                                              style={{ color: "red" }}
                                              onClick={() =>
                                                handleUnlike(idUser, product.id)
                                              }
                                            />
                                          ) : (
                                            <FontAwesomeIcon
